import json
from config import JSONS_CONFIG_PATH, JSON_TASKS_PATH, JSON_REPETITIVE_TASKS_PATH
import datetime
...
def add_task():
    ...
    if date_ == 'today':
        date_ = datetime.date.today().strftime("%d-%m-%Y")
...
def display_tasks_for_today():
    ...
    for key in tasks_data[datetime.date.today().strftime("%d-%m-%Y")]:
        output = '| '
        ...
        text = tasks_data[datetime.date.today().strftime("%d-%m-%Y")][key]['text']

        while len(text) > 41:
            text = text.lstrip()

            if tasks_data[datetime.date.today().strftime("%d-%m-%Y")][key]['text'] != text:
                output += '|     '
